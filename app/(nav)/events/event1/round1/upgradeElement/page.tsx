...
export default function Testing() {
    ...
    return (
        <div className="relative w-full h-full min-h-screen bg-gray-100">
            <div className="fixed inset-0 overflow-hidden">
                <Image src={bg} alt="bg" className="absolute inset-0 w-full h-full object-cover opacity-40" priority />
            </div>
            
            <div className="container mx-auto p-10 text-center relative z-10">
                <h1 className="text-4xl font-extrabold mb-8 mt-24 text-black">Choose Performance Upgrade</h1>
                <h6 className="text-lg font-medium text-gray-800 mb-8">This upgrade will affect your performance for the next 10 minutes.</h6>
                <p className="text-xl font-bold mb-6">Current Rate: {rate !== null ? rate : "Loading..."}</p>
            ...
            {selectedUpgrade && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-50">
                    <div className="bg-white p-8 shadow-2xl text-center max-w-md mx-4 rounded-2xl">
                        <h2 className="text-2xl font-bold mb-6">Confirm Upgrade</h2>
                        <p className="text-xl font-bold text-red-700 mb-2">Upgrade {selectedUpgrade.id}</p>
                        <p className="text-2xl font-bold mb-4">{selectedUpgrade.name}</p>
                        <p className="text-xl font-bold">Cost: ₹ {selectedUpgrade.cost.toLocaleString()}</p>
                        <div className="mt-6 flex justify-center gap-6">
                            <button onClick={() => setSelectedUpgrade(null)} className="px-6 py-3 bg-gray-200 rounded-xl">Cancel</button>
                            <button onClick={handleUpgrade} className="px-6 py-3 bg-red-600 text-white rounded-xl">Confirm</button>
                        </div>
                    </div>
                </div>
            )}
