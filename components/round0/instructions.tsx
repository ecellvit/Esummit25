...
const Instructions: React.FC = () => {
  ...
  const targetDate: string = new Date(2025, 2, 27, 11, 58, 0).toISOString();
    ...
    <main className="min-h-[100vh] text-black flex flex-col items-center">
      <div className="flex flex-col items-start w-[90vw] md:w-[80vw] lg:w-[70vw] px-8 py-4 border rounded-xl m-2 text-lg md:text-xl">
        ...
        <h1 className="font-bold text-2xl md:text-3xl mb-4">
          Welcome to the Qualifying Round of E-Summit!
        </h1>
      ...
      <button
        className="px-4 py-2 rounded-full text-white bg-gradient-to-r from-red-500 to-white-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none m-4 w-full max-w-md h-12 flex items-center justify-center font-bold hover:opacity-80 transition-opacity duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
        onClick={startQuiz}
        disabled={loading}
      >
